    ...
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40 px-8 py-4 flex justify-between items-center shadow-sm">
        ...
        <div className="space-x-4">
          <Link href={`/${params.tenant}/login`} className="text-sm font-bold text-slate-600 hover:text-slate-900">Iniciar Sesión</Link>
        </div>
      </header>
        ...
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row gap-12 items-center">
          <div className="flex-1 space-y-6">
            ...
            <div className="flex items-center gap-6 pt-4 text-sm font-bold text-slate-300">
               <span className="flex items-center"><Clock className="h-5 w-5 mr-2 text-emerald-500" /> 12 Horas de Contenido</span>
               <span className="flex items-center"><Play className="h-5 w-5 mr-2 text-emerald-500" /> {course._count.sections} Módulos</span>
            </div>
          </div>
          ...
          <div className="w-full md:w-[400px] bg-white rounded-3xl p-6 shadow-2xl text-slate-900 transform md:translate-y-12 border border-slate-100 relative z-10">
             ...
             <div className="space-y-4">
                <div className="flex items-end gap-2 mb-2">
                   <span className="text-4xl font-black tracking-tight">${course.price ? course.price.toLocaleString('es-CL') : '0'}</span>
                   <span className="text-slate-500 font-medium mb-1 line-through">${(course.price * 1.5).toLocaleString('es-CL')}</span>
                </div>
                <Link 
      ...
      <div className="max-w-5xl mx-auto px-8 py-20 pb-40">
        <div className="md:w-2/3 pr-12">
          ...
          <div className="space-y-4">
             {/* Esqueleto de secciones ilustrativo si no hemos cargado secciones aún */}
             {[1, 2, 3].map((sectionData) => (
               <div key={sectionData} className="border border-slate-200 rounded-2xl bg-white overflow-hidden shadow-sm">
                  <div className="p-5 bg-slate-50 border-b border-slate-200 font-bold text-slate-900 flex justify-between items-center cursor-pointer hover:bg-slate-100 transition-colors">
                     Módulo {sectionData}: Introducción y Conceptos Básicos
